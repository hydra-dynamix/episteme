...
The substrate is built as one logical progression of four experimentally motivated refinements. First, canonical recurrence indexing. We search by node identity by first-occurrence position not semantic labels. This retrieves bounded basins of candiates rather than single answers. Second, adding typed relation labels, stored and indexed by the topological key signature, sharpens a discrimination that pure topology cannot provide: polysemy disambiguation rises from 0.25 to 1.0. Third, when neutral noise collapsed retrieval, an identity-regime smoke test attributed the failure to the *matcher*, not the *representation*; replacing a greedy with a DP/LCS alignment moved the noise transition from magnitude 2 to 3 without touching identity. Fourth, storing the semantic connections inside each keyed motif as a payload graph then escapes the flat node-bag ceiling: same-node rewired decoys (identical node sets and relation-label multisets, different connections) defeat node-bag matching (top-1 0.0) but not content-graph matching (top-1 1.0 on structured queries).
